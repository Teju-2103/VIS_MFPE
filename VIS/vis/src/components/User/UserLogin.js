...
class UserLogin extends Component {
    ...
    submitHandler(event) {
        ...
        const apiurl="http://localhost:64277/api/User/Login";
        axios.post(apiurl,data).then((response)=>{
            console.log(response.data);
            if(response.data.statusCode===200){
              const userId=sessionStorage.setItem('userId',response.data.userId);
              this.setState({redirect:"/Dashboard/UserDashboard"});  
            }
            else{
        ...
        <div className="row">
            ...
            style={{padding:"40px", backgroundColor:"#263341", borderRadius:"15px"}}>
                <Form onSubmit={this.submitHandler}>
                    ...
                    <p className='text-center'>Login to your VIS account</p>
                   ...
                   <Form.Control.Feedback type="invalid"><b>{ errors.userName }</b></Form.Control.Feedback>
                    ...
                    <Link to="/User/register">
                        <Button className="m-4" variant="danger">Join Us</Button>
                    </Link>
                    </div>
                </Form>
            </div>
